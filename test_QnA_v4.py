...
def post_process(prompt, dialog, model_name, info):
    dialog=dialog[-3:]
    if len(dialog) == 1:
        dialog= ["Bom dia!","Bom dia, como posso ajudá-los?"] + dialog
    dialog_text=""
    for i in range(len(dialog)):
        if i % 2 == 0:
            dialog_text = dialog_text + "User: " + dialog[i]+"\n"
    ...
    for i in range(n):
        if df.iloc[i]["similarities"] > minimum_similarity:
            if df["Answers"].iloc[i] not in top_answer:
                top_answer = top_answer + " " + df["Answers"].iloc[i].strip()
